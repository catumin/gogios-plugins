...
# Packaging variable
PACKAGE_NAME = "gogios-plugins"
USER = "gogios"
GROUP = "gogios"
PLUGIN_DIR = "/usr/lib/gogios/plugins"

# META-PACKING VARIABLES
PACKAGE_LICENSE = "MIT"
PACKAGE_URL = "https://github.com/BKasin/gogios-plugins"
MAINTAINER = "bailey@gingertechnology.net"
VENDOR = "GingerTechnology"
DESCRIPTION = "Plugins for the gogios checking system."
...
go_targets = {
    'check-dns': './plugins/check-dns',
    'check-ftp': './plugins/check-ftp',
    'check-host-alive': './plugins/check-host-alive',
    'check-mysql': './plugins/check-mysql',
    'check-pop3': './plugins/check-pop3',
    'check-postgresql': './plugins/check-postgresql',
    'check-smtp': './plugins/check-smtp',
    'check-ssh': './plugins/check-ssh',
}

copy_targets = {
    'check-tcp-port': './plugins/check-tcp-port/check-tcp-port',
}
...
def package_scripts(build_root):
    """Copy plugins that do not need to be compiled.
    """
    logging.info("Copying script plugins to the build directory")
    for target, path in copy_targets.items():
        shutil.copyfile(path, os.path.join(build_root, PLUGIN_DIR[1:], target))
        os.chmod(os.path.join(build_root, PLUGIN_DIR[1:], target), 0o755)


def go_get(branch, update=False, no_uncommitted=False):
    """Retrieve build deps or restore pinned deps.
    """
    if local_changes() and no_uncommitted:
        logging.error(
            "There are uncommitted changes in the current directory.")
        return False
    logging.info("Retrieving deps...")
    run("go mod download")
...
def run(command, allow_failure=False, shell=False):
    """Run shell command (subprocess wrapper).
    """
...
def increment_minor_version(version):
    ...
    logging.debug("Incremented version from '{}' to '{}'.".format(
        version, inc_version))
    return inc_version
...
def check_environ(build_dir=None):
    ...
    if build_dir is None and os.environ.get("GOPATH") and os.environ.get("GOPATH") not in cwd:
        logging.warn(
            "Your current directory is not under your GOPATH. This may lead to build failures.")
    return True
...
def go_list(vendor=False, relative=False):
    ...
    If vendor is False vendor packages are not included
    If relative is True the package prefix defined by PACKAGE_URL is stripped
    """
    p = subprocess.Popen(["go", "list", "./..."],
...
def build(version=None,
    ...
    if not os.path.exists(outdir):
        ...
        logging.info(
            "Cleaning build directory '{}' before building.".format(outdir))
    ...
    for target, path in go_targets.items():
        logging.info("Building target: {}".format(target))
        build_command = ""

        # Handle static binary ouput
        ...
        if "arm" in arch:
            if arch == "armel":
                ...
                logging.error(
                    "Invalid ARM architecture specified: {}".format(arch))
                logging.error(
                    "Please specify either 'armel', 'armhf', or 'arm64'.")
                return False
...
def generate_sha256_from_file(path):
    """Generate a SHA256 hash signature based on the contents of the file
    at path.
    """
    m = hashlib.sha256()
...
def generate_sig_from_file(path):
    ...
    """
    logging.debug("Generate GPG sig for file: {}".format(path))
    gpg_path = check_path_for('gpg')
    if gpg_path is None:
        logging.warn(
            "gpg binary was not found in PATH! Skipping signature creation.")
        return False
    ...
    try:
        for platform in build_output:
            ...
            for arch in build_output[platform]:
                ...
                # Create diretory tree to mimic file system of package
                ...
                for binary in go_targets:
                    ...
                    if static or "static_" in arch:
                        ...
                        fr = os.path.join(current_location, binary)
                        to = os.path.join(build_root, PLUGIN_DIR[1:], binary)
                    shutil.copy(fr, to)
...
def main(args):
    ...
    if args.release and args.nightly:
        logging.error("Cannot be a nightly and a release.")
        return 1
    ...
    if args.branch != orig_branch and args.commit != arig_commit:
        logging.error("Can only specify ones branch or commit.")
        return 1
        ...
        if args.sign:
            ...
            for p in packages:
                if generate_sig_from_file(p):
                    ...
                    logging.error(
                        "Creation of signature for package [{}] failed!".format(p))
                    return 1
            packages += sigs
